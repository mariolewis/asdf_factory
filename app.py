...
if page == "Project":
    # (Project page code remains the same)
    if not st.session_state.orchestrator.project_id:
        ...
        if st.button("Start New Project"):
            if project_name_input:
                st.session_state.messages.append({"role": "assistant", "content": f"Welcome! Starting project '{project_name_input}'."})
        ...
        if prompt := st.chat_input("What would you like to do?"):
            ...
            with st.chat_message("assistant"):
                response = f"Acknowledged. You said: '{prompt}'. The factory is not yet configured to respond."
