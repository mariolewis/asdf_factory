...
class MasterOrchestrator:
    ...
    def handle_intake_assessment_decision(self, decision: str, **kwargs):
        ...
        if decision == "FULL_LIFECYCLE":
            ...
            # 1. Set the cursor to Wait (Spinning Wheel)
            QApplication.setOverrideCursor(Qt.WaitCursor)

            # 2. Force the UI to repaint immediately
            # This is the "Magic Line" that ensures the cursor changes BEFORE the freeze starts.
            ...
            try:
                ...
                if not app_spec or app_spec.strip().startswith("Error:"):
                    raise ValueError(f"The AI failed to synthesize an Application Specification. Details: {app_spec}")
                ...
                # 3. Restore the normal cursor stack
    ...
    def _execute_source_code_generation_task(self, task: dict, project_root_path: Path, db: KlyveDBManager, progress_callback=None):
        ...
        """
        # --- ROUTING LOGIC: Enforce Controlled Vocabulary ---
        component_type = task.get("component_type", "source_code_generation")

        # 1. Project Configuration (CLI Commands) - Delegate & Return
        if component_type == "project_configuration":
            self._handle_project_configuration_task(task, project_root_path, progress_callback)
            return

        # 2. Static Resources (No Review Needed) - Delegate & Return
        if component_type == "static_resource":
            self._handle_static_resource_task(task, project_root_path, progress_callback)
            return

        # 3. Safety Catch: Warn if unknown, but proceed to standard Source Code Gen logic below
        if component_type != "source_code_generation":
             logging.warning(f"Unknown component_type '{component_type}' for {task.get('component_name')}. Proceeding as source_code_generation.")
        ...
        # Ensure test_path is a string, even if the task provides None
        safe_test_path = test_path if test_path else ""
        ...
        status, result_message = build_agent.build_and_commit_component(
            task.get("component_file_path"), # Arg 1: Component Path
            source_code,                     # Arg 2: Source Code
            safe_test_path,                  # Arg 3: Test Path (Safe)
            unit_tests,                      # Arg 4: Test Code
            test_command,                    # Arg 5: Test Command
            self.llm_service,                # Arg 6: LLM Service
            version_control_enabled          # Arg 7: VCS Flag
        )
    ...
    def _handle_project_configuration_task(self, task: dict, project_root_path: Path, progress_callback=None):
        """
        Handles tasks labeled 'project_configuration' via CLI execution.
        """
        import subprocess
        import hashlib
        import git
        ...
        logging.info(f"Handling configuration task for {component_name} via CLI execution.")
        if progress_callback:
            progress_callback(("INFO", f"Configuring project structure for: {component_name}..."))

        try:
            if not self.llm_service:
                raise Exception("LLM Service not initialized.")

            # 1. Fetch Project Details (Strictly using [] notation for sqlite3.Row)
            project_details = self.db_manager.get_project_by_id(self.project_id)

            # Handle Target OS safely without using .get()
            target_os = project_details['target_os']
            if not target_os:
                target_os = 'Linux' # Default fallback

            code_agent = CodeAgent_AppTarget(llm_service=self.llm_service)

            # Instruction for the agent
            instruction = (
                f"Write the specific terminal command (CLI) to achieve this: {task_desc}\n"
                f"Target OS: {target_os}"
            )

            # Generate the command string
            command_str = code_agent.generate_code_for_component(
                logic_plan=instruction,
                coding_standard="Output ONLY the raw single-line shell command. Do not use markdown formatting.",
                style_guide=None
            )

            # Clean up any potential markdown fences
            command_str = command_str.replace("```bash", "").replace("```sh", "").replace("```", "").strip()

            logging.info(f"Executing Configuration Command: {command_str}")
            if progress_callback:
                progress_callback(("INFO", f"Running command: {command_str}"))

            # 2. Execute the command in the project root
            ...
            if result.returncode != 0:
                # --- NEW LOGIC START ---
                # Check for environment-specific errors (command not found, etc.)
                err_msg = result.stderr.lower()
                if "not recognized" in err_msg or "command not found" in err_msg or "no such file" in err_msg:

                    # 1. Prepare the failure context for the UI
                    self.task_awaiting_approval = {
                        "failure_log": f"Command: {command_str}\nError: {result.stderr}",
                        "is_env_failure": True # This triggers the correct UI page
                    }

                    # 2. Set the phase to escalate immediately
                    self.set_phase("DEBUG_PM_ESCALATION")

                    # 3. Raise the specific exception to stop the worker safely
                    raise EnvironmentFailureException(f"Environment Tool Missing.\nCommand: {command_str}\nError: {result.stderr}")
                # --- NEW LOGIC END ---

                raise Exception(f"Configuration command failed.\nCommand: {command_str}\nError: {result.stderr}")

            logging.info(f"Command output: {result.stdout}")

            # 3. Calculate File Hash
            file_path = project_root_path / task.get("component_file_path")
            file_hash = ""
            if file_path.exists():
                try:
            ...
            # 4. Commit & Get Commit Hash
            commit_hash = ""
            # Use the already fetched project_details
            version_control_enabled = project_details['version_control_enabled'] == 1

            if version_control_enabled:
                if progress_callback: progress_callback(("INFO", "Committing generated configuration files..."))
                build_agent = BuildAndCommitAgentAppTarget(str(project_root_path), version_control_enabled=True)
                commit_msg = f"config: Execute project configuration for {component_name}"

                success, msg = build_agent.commit_all_changes(commit_msg)

                if not success:
                    logging.warning(f"Commit warning during configuration: {msg}")

                try:
                    repo = git.Repo(str(project_root_path))
                    commit_hash = repo.head.commit.hexsha
                except Exception:
                    logging.warning("Could not retrieve commit hash.")

            # 5. Update RoWD
            doc_agent = DocUpdateAgentRoWD(self.db_manager, llm_service=self.llm_service)
            doc_agent.update_artifact_record({
                "artifact_id": task.get("artifact_id") or f"art_{uuid.uuid4().hex[:8]}",
                "project_id": self.project_id,
                "file_path": task.get("component_file_path"),
                "artifact_name": component_name,
                "artifact_type": "CONFIGURATION",
                "short_description": task_desc,
                "status": "COMPLETED",
                "unit_test_status": "NOT_APPLICABLE",
                "version": 1,
                "last_modified_timestamp": datetime.now(timezone.utc).isoformat(),
                "micro_spec_id": task.get("micro_spec_id"),
                "code_summary": f"Executed command: {command_str}",
                "file_hash": file_hash,
                "commit_hash": commit_hash
            })

            if progress_callback:
                progress_callback(("SUCCESS", f"Configuration task '{component_name}' completed successfully."))

        except Exception as e:
            # We log here, but we must re-raise so the main loop knows to stop.
            # If it's an EnvironmentFailureException, the main loop catches it and respects the state we set above.
            logging.error(f"Configuration task failed: {e}", exc_info=True)
            raise e

    def _handle_static_resource_task(self, task: dict, project_root_path: Path, progress_callback=None):
        """
        Handles generation of static files (README, .gitignore, JSON config).
        Distinguishes between System Errors (Permissions/Git) and Content Errors.
        """
        import uuid
        import hashlib
        import git
        ...
        component_name = task.get("component_name")
        if progress_callback:
            progress_callback(("INFO", f"Generating static resource: {component_name}"))
        ...
        try:
            ...
            # 2. Generate Content (Logic/AI Step - Can trigger Auto-Fix if it fails logic checks)
            code_agent = CodeAgent_AppTarget(llm_service=self.llm_service)

            content = code_agent.generate_code_for_component(
                logic_plan=task.get("task_description"),
                coding_standard="Output raw text/json/xml/yaml content only. No markdown formatting.",
                style_guide=None
            )

            content = content.replace("```json", "").replace("```xml", "").replace("```yaml", "").replace("```", "").strip()
            file_hash = hashlib.sha256(content.encode('utf-8')).hexdigest()

            # 3. Write to File (System Step - Vulnerable to Environment Failures)
            full_path = project_root_path / task.get("component_file_path")
            try:
                full_path.parent.mkdir(parents=True, exist_ok=True)
                with open(full_path, "w", encoding="utf-8") as f:
                    f.write(content)
            except OSError as io_error:
                # Catch PermissionError, DiskFull, etc.
                logging.error(f"System I/O Failure for {component_name}: {io_error}")
                # We raise a new EnvironmentFailureException to bypass Auto-Fix
                self.task_awaiting_approval = {
                    "failure_log": f"File System Error: {io_error}",
                    "is_env_failure": True
                }
                self.set_phase("DEBUG_PM_ESCALATION")
                raise EnvironmentFailureException(f"File System Error: {io_error}")

            # 4. Commit & Get Commit Hash (System Step - Vulnerable to Git Failures)
            commit_hash = ""
            version_control_enabled = project_details['version_control_enabled'] == 1
            ...
            if version_control_enabled:
                try:
                    build_agent = BuildAndCommitAgentAppTarget(str(project_root_path), version_control_enabled=True)
                    commit_msg = f"docs: Add static resource {component_name}"

                    success, msg = build_agent.commit_all_changes(commit_msg)

                    if not success:
                        logging.warning(f"Commit warning during static resource generation: {msg}")

                    repo = git.Repo(str(project_root_path))
                    commit_hash = repo.head.commit.hexsha
                except Exception as git_error:
                    # Git errors are often environment issues (locks, corrupt repo)
                    # We log warning but don't hard crash the task unless strictly necessary.
                    # If you prefer strictness, you could raise EnvironmentFailureException here too.
                    logging.warning(f"Git Environment Warning: {git_error}")
            ...
            doc_agent.update_artifact_record({
                ...
                "last_modified_timestamp": datetime.now(timezone.utc).isoformat(),
                "micro_spec_id": task.get("micro_spec_id"),
                "code_summary": "Static resource generated.",
                "file_hash": file_hash,
                "commit_hash": commit_hash
            })

            if progress_callback:
                progress_callback(("SUCCESS", f"Static resource {component_name} created."))

        except EnvironmentFailureException as e:
            # Let specific environment failures pass through to the main loop's handler
            raise e
        except Exception as e:
            # Catch ALL other errors (e.g., Agent generation failures)
            # These will go to the Auto-Fix loop because they might be logic/prompt issues
            logging.error(f"Static resource task failed: {e}", exc_info=True)
            raise e
    ...
    def verify_connection_with_params(self, settings: dict) -> tuple[bool, str]:
        """
        Attempts to create a temporary LLM service and run a test prompt
        using the provided settings dictionary (bypassing the database).
        """
        ...
        try:
            ...
            # 1. Instantiate the adapter using the dictionary values
            if provider_name == "Gemini":
                temp_service = GeminiAdapter(
                    settings.get("GEMINI_API_KEY"),
                    settings.get("GEMINI_REASONING_MODEL"),
                    settings.get("GEMINI_FAST_MODEL")
                )
            elif provider_name == "ChatGPT":
                temp_service = OpenAIAdapter(
                    settings.get("OPENAI_API_KEY"),
                    settings.get("OPENAI_REASONING_MODEL"),
                    settings.get("OPENAI_FAST_MODEL")
                )
            elif provider_name == "Claude":
                temp_service = AnthropicAdapter(
                    settings.get("ANTHROPIC_API_KEY"),
                    settings.get("ANTHROPIC_REASONING_MODEL"),
                    settings.get("ANTHROPIC_FAST_MODEL")
                )
            elif provider_name == "Grok":
                temp_service = GrokAdapter(
                    settings.get("GROK_API_KEY"),
                    settings.get("GROK_REASONING_MODEL"),
                    settings.get("GROK_FAST_MODEL")
                )
            elif provider_name == "Deepseek":
                temp_service = DeepseekAdapter(
                    settings.get("DEEPSEEK_API_KEY"),
                    settings.get("DEEPSEEK_REASONING_MODEL"),
                    settings.get("DEEPSEEK_FAST_MODEL")
                )
            elif provider_name == "Llama":
                temp_service = LlamaAdapter(
                    settings.get("LLAMA_API_KEY"),
                ...
                temp_service = CustomEndpointAdapter(
                    ...
                    settings.get("CUSTOM_ENDPOINT_API_KEY"),
                    settings.get("CUSTOM_REASONING_MODEL"),
                    settings.get("CUSTOM_FAST_MODEL")
                )
            else:
                return False, f"Unknown provider selected: {provider_name}"

            # 2. Run a test generation to validate Key AND Model Name
            test_response = temp_service.generate_text("Reply with the single word: Success", task_complexity="simple")
