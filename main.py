...
def initialize_database(db_manager: ASDFDBManager):
    ...
    This is run once at application startup.
    """
    ...
    with db_manager as db:
        ...
        for key, (value, desc) in defaults.items():
            if key not in all_config:
                db.set_config_value(key, value, desc)
                logging.info(f"Initialized missing config key '{key}' with default value.")

if __name__ == "__main__":
    # 1. Basic App and Path Setup
    app = QApplication(sys.argv)
    db_dir = Path("data")
    db_dir.mkdir(exist_ok=True)
    db_path = db_dir / "asdf.db"

    # 2. Create the SINGLE database manager instance for the whole application
    db_manager = ASDFDBManager(db_path=str(db_path))

    # 3. Initialize Database and Defaults using the single instance
    ...
    # 5. Create Core Components, passing the SINGLE db_manager to the orchestrator
    orchestrator = MasterOrchestrator(db_manager=db_manager)
    window = ASDFMainWindow(orchestrator=orchestrator)

    # 6. Load Stylesheet and Show Window
