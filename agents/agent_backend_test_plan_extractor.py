...
class BackendTestPlanExtractorAgent:
    ...
    def extract_plan(self, technology_stack: str, test_files_content: dict) -> str:
        """
        ...
        """
        logging.info(f"Extracting backend test plan from {len(test_files_content)} {technology_stack} files...")

        # Format the code content for the prompt
        code_context = ""
        for path, content in test_files_content.items():
            code_context += f"--- File: {path} ---\n```{technology_stack.lower()}\n{content}\n```\n\n"

        try:
            prompt = textwrap.dedent(f"""
                You are an expert QA Engineer who specializes in reverse-engineering test plans from existing code. Your task is to analyze a set of test files for a '{technology_stack}' project and produce a structured JSON test plan.
                ...
                **--- INPUT: Test Code Files (Language: {technology_stack}) ---**
                {code_context}
