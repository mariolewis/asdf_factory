...
class ProjectSettingsDialog(QDialog):
    ...
    def accept(self):
        """
        Overrides the standard accept slot to perform security validation.
        """
        # Validate Test Commands (Allow shell operators, block SQLi)
        commands = [
            self.ui.backendTestCommandLineEdit.text(),
            self.ui.integrationTestCommandLineEdit.text(),
            self.ui.uiTestCommandLineEdit.text()
        ]
        for cmd in commands:
            if cmd.strip():
                if not validate_security_input(self, cmd, "COMMAND"):
                    return

        # Validate Jira Fields if active
        if self.ui.providerComboBox.currentText() == "Jira":
            jira_inputs = [
                self.ui.projectKeyLineEdit.text(),
                self.ui.epicTypeIdLineEdit.text(),
                self.ui.storyTypeIdLineEdit.text(),
                self.ui.taskTypeIdLineEdit.text(),
                self.ui.bugTypeIdLineEdit.text(),
                self.ui.changeRequestTypeIdLineEdit.text()
            ]
            for val in jira_inputs:
                if val.strip():
                    # We use 'NAME' mode for strict validation (blocks path chars and shell operators)
                    if not validate_security_input(self, val, "NAME"):
                        return
        super().accept()